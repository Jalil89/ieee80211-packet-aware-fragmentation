...
/*
 * @author Jalil
 * flow info struct to get the packet error rate
 */
struct flow_info{
	struct list_head list;
	unsigned int dest_addr;
	int avg_pkt_size; // the rate of queue in bps;
	int total_pkt; // total packets sent in an A-MPDU
	int total_pkt_succ; // total packets received successfully
};
...
struct ieee80211_ops {
	void (*get_flow_info) (struct flow_info *fi, unsigned int dst_addr);
	void (*register_new_flow) (unsigned int dst_addr);
