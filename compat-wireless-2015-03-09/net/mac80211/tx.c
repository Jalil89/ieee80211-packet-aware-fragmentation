					   ...
					   struct sta_info **sta_out)
{
	...
	if (ieee80211_is_data_qos(fc)) {
		__le16 *qos_control;
		if (skb->priority > 0){
			//printk(KERN_ALERT "qos enabled. priority = %u", skb->priority);
		}
		...
		*qos_control = 0;
		...
		if (skb->priority > 0){
			//printk(KERN_ALERT "qos disabled. priority = %u", skb->priority);
		}
	...
	/*
	if (skb->priority == 7){
			//printk(KERN_ALERT "qos disabled for the data");
			skb->priority = 0;
	}
	*/
...
netdev_tx_t __ieee80211_subif_start_xmit(struct sk_buff *skb,
				  struct net_device *dev,
				  u32 info_flags)
{
 ...
 */
netdev_tx_t ieee80211_subif_start_xmit_frags_no_ret(struct sk_buff *skb,
				       struct net_device *dev)
{
...
netdev_tx_t ieee80211_subif_start_xmit_frags(struct sk_buff *skb,
				       struct net_device *dev)
{
	return  __ieee80211_subif_start_xmit(skb, dev, 0);
}
...
netdev_tx_t ieee80211_form_and_tx(struct sk_buff *skb,
		struct net_device *dev, int start_pos, int payload_len , int tid){
	...
	ptr_payload = payload + ETH_HLEN + IP_HDR_LEN + UDP_HDR_LEN + OVERHEAD_TS + OVERHEAD_PARAM;
	// copy the RTP header
	memcpy(ptr_temp, ptr_payload,RTP_LEN);
	//advance to the payload (TS packet)
	ptr_temp = ptr_temp + RTP_LEN;
	ptr_payload = ptr_payload + RTP_LEN + start_pos;
	// copy the TS packet
	...
	kfree(temp);
	// set priority to AC_VI
	int ac = ieee802_1d_to_ac[tid & 7];
	skb_set_queue_mapping(new_skb, ac);
	new_skb->priority = tid;
	rh = (struct rtp_hdr*) skb_header_pointer (new_skb, ETH_HLEN+IP_HDR_LEN+UDP_HDR_LEN, 0, NULL);
	//printk(KERN_ALERT "RTP seq num is new %u\n", rh->rtpsqnum);
	// finally transfer the fragment
	return ieee80211_subif_start_xmit_frags(new_skb,dev);
}


unsigned int get_opt_frag_size(struct net_device *dev, unsigned int d_addr){
	struct flow_info fi;
	struct ieee80211_sub_if_data *sdata;
	struct ieee80211_local *local;
	unsigned int opt_size = 1500;
	fi.dest_addr = 0;
	sdata = IEEE80211_DEV_TO_SUB_IF(dev);
	local = sdata->local;
	local->ops->get_flow_info(&fi,d_addr);
	if (fi.dest_addr){
		// calculate here
		opt_size = get_optimal_frag_size(fi.avg_pkt_size, fi.total_pkt, fi.total_pkt_succ);
		//printk(KERN_ALERT "avg pkt size : %u \n", fi.avg_pkt_size);
		//printk(KERN_ALERT "total pkts : %d \n", fi.total_pkt);
		//printk(KERN_ALERT "total pkts successful : %d \n", fi.total_pkt_succ);
		//printk(KERN_ALERT "optimal frag size : %d \n", opt_size);
	}else{
		local->ops->register_new_flow(d_addr);
	}
	return opt_size;
}
...
netdev_tx_t ieee80211_proposed_tx(struct sk_buff *skb,
				struct net_device *dev){
	struct bitmap_t b;
	struct iphdr *iph;
	struct tshdr *tsh;
	...
	struct param_hdr *ph;
	u8 *payload;
	unsigned int data_len, opt_frag_size;
	...
	if (iph && iph->protocol &&(iph->protocol == IPPROTO_UDP)) {
		...
		if (IS_TS(tsh)){
			...
			int i;
			for (i = 0; i < 8; i++){
				printk(KERN_ALERT "%d", !!((tsh->bitmap << i) & 0x80));

			}
			*/
			//printk(KERN_ALERT "\n");
			//opt_frag_size = get_opt_frag_size(dev, iph->daddr);
			ph = (struct param_hdr*) skb_header_pointer (skb, ETH_HLEN+IP_HDR_LEN+UDP_HDR_LEN + OVERHEAD_TS, 0, NULL);
			rh = (struct rtp_hdr*) skb_header_pointer (skb, ETH_HLEN+IP_HDR_LEN+UDP_HDR_LEN + \
					OVERHEAD_TS + OVERHEAD_PARAM, 0, NULL);
			rtp_num = rh->rtpsqnum ;
			opt_frag_size = ph -> frag_size;
			//printk(KERN_ALERT "RTP seq num is original %u\n", rh->rtpsqnum);
			//printk(KERN_ALERT "opt pkt size : %u \n", opt_frag_size);
			//printk(KERN_ALERT "prioritize : %u \n",ph->prioritize);
			while(bitmap_has_next(&b)){
				// set up RTP number for deaggregation at the decoder
				rh->rtpsqnum = (rtp_num * BITMAP_MAX_SIZE + b.curr_pos) % 65535;
				// update the bitmap for the next chunk
				bitmap_get_next(&b,opt_frag_size);
				payload_len = b.chunk_length * TS_PSIZE;
				int tid = 0;
				if (bitmap_is_high_priority(&b)){
					tid = 5; //AC_VI
					//printk(KERN_ALERT "high - %d\n", b.chunk_length);
				}
				else {
					tid = 3 ; //AC_BE
					//printk(KERN_ALERT "low - %d\n", b.chunk_length);
				}
				res = ieee80211_form_and_tx(skb, dev, b.curr_pos * TS_PSIZE , payload_len ,tid);
				while (res != NETDEV_TX_OK){ // check the result
		...
		return ieee80211_subif_start_xmit_frags_no_ret(skb,dev);
}



netdev_tx_t ieee80211_existing_tx(struct sk_buff *skb,
				struct net_device *dev){
	...
	struct iphdr *iph;
	struct udphdr *udph;
	unsigned int  opt_frag_size;
	unsigned int payload_len, IP_PSIZE; // length of data accupied by only TS packets
	unsigned int payload_start = 0;
	IP_PSIZE = TS_PSIZE * 7;
	netdev_tx_t  res = NETDEV_TX_OK;
	opt_frag_size = 329; // initialize
	...
	if (iph && iph->protocol &&(iph->protocol == IPPROTO_UDP)) {
		tsh = (struct tshdr*) skb_header_pointer (skb, ETH_HLEN+IP_HDR_LEN+UDP_HDR_LEN, 0, NULL);
		if (IS_TS(tsh)){
			ph = (struct param_hdr*) skb_header_pointer (skb, ETH_HLEN+IP_HDR_LEN+ \
						UDP_HDR_LEN + OVERHEAD_TS, 0, NULL);
			opt_frag_size = ph->frag_size;
			//printk(KERN_ALERT "opt pkt size : %u \n", opt_frag_size);
			while (IP_PSIZE > 0){
				// set up RTP number for deaggregation at the decoder
				if (IP_PSIZE > opt_frag_size){
					payload_len = opt_frag_size;
				}
				else{
					payload_len = IP_PSIZE;
				}
				IP_PSIZE -= payload_len;
				int tid = 3;
				res = ieee80211_form_and_tx(skb,dev, payload_start,payload_len,tid);
				while (res != NETDEV_TX_OK){ // check the result
					printk(KERN_ALERT "TX not successful\n");
					res = ieee80211_form_and_tx(skb,dev, payload_start,payload_len, tid);
				}
...
/*
 * @author: Jalil
 * The starting position for MAC layer packet
 */
	...
	if (iph && iph->protocol &&(iph->protocol == IPPROTO_UDP)) {
		...
		if (IS_TS(tsh)){
			ph = (struct param_hdr*) skb_header_pointer (skb, ETH_HLEN+IP_HDR_LEN+ \
						UDP_HDR_LEN + OVERHEAD_TS, 0, NULL);
			if (ph->prioritize == 1){
				return ieee80211_proposed_tx(skb,dev);
			}
			else{
				return ieee80211_existing_tx(skb,dev);
			}
		}
	}
	done:
